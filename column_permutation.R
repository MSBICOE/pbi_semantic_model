...
library(gtools)
#urn with 3 balls
ccc <- c("Budget.impact", "Cost.min", "Cost.effectiveness", "high.unmet.need.", "Submission.type")

x <- read_excel("data/pbs_2.xlsx", sheet = "s1", 
...
x <- read_excel("data/pbs_2.xlsx", sheet = "s2", 
                col_types = c("text", "text", "text", "text", "text", "text", "text", "text", "text", "text", "text", "numeric", 
...
para_x <- x.3 %>% select("Budget.impact", "Cost.min", "Cost.effectiveness", "high.unmet.need.", "Submission.type") %>% 
  distinct() %>% 
  complete(TA.to.match, Budget.impact, Cost.min, Cost.effectiveness, high.unmet.need., Submission.type)
  
x.3 %>% 
  inner_join(x.4) %>% View()


select(x.3, Budget.impact) %>% unique()

select(x.3, Cost.min) %>% unique()

select(x.3, Cost.effectiveness) %>% unique()
select(x.3,high.unmet.need.) %>% unique()
select(x.3, Submission.type) %>% unique()
  
  # replace(ifelse(. == "UUnk", TRUE, FALSE), "Test")
...
library(gtools)
#urn with 3 balls
x <- c("Budget.impact", "Cost.min", "Cost.effectiveness", "high.unmet.need.", "Submission.type")
#pick 2 balls from the urn with replacement
#get all permutations
k <- 3
permutations(n=5,r=k,v=x)





x.4[, c("Budget.impact", "Cost.min", "Cost.effectiveness", "high.unmet.need.", "Submission.type")] <- "Test"
